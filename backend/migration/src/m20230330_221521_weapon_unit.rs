use entity::{sea_orm::{EntityTrait, Schema, DbBackend}, weapon_unit};
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let stmts = vec![get_seaorm_create_stmt(weapon_unit::Entity)];
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        let stmts = vec![get_seaorm_drop_stmt(weapon_unit::Entity)];
